...
from l_cache import (
    u_l_cache, l_user_cache, CacheKeyEnum, CacheType, StorageType,
...
class TestULCacheDecorator:
    ...
    def test_make_ctx_cache_sign(self):
        """测试生成上下文缓存签名"""
        decorator = u_l_cache()
        sign1 = decorator.make_ctx_cache_sign()
        sign2 = decorator.make_ctx_cache_sign()
        
        assert isinstance(sign1, str)
        assert len(sign1) > 0
        assert sign1 == sign2  # 应该是一致的
...
class TestLUserCacheDecorator:
    ...
    def test_init_with_custom_params(self):
        ...
        assert decorator.prefix == "custom:"
